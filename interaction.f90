...
SUBROUTINE setup_2b_interaction
  ...
  COMPLEX(dpc), ALLOCATABLE :: fock_mtx0(:,:)

  CALL assert_built('channels_2b', 'setup_2b_interaction')
  CALL assert_built('mappings',    'setup_2b_interaction')
  CALL assert_holes_ordered('setup_2b_interaction')
  ...
  !    v2_mem0 = v2_mem0 + (bra_confs)**2 * 16.d0 ! memory
  ! end DO
  ...
  DO ch = 1, channels_2b%number_confs
     ...
     CALL mpi_allreduce(mpi_in_place,v2b_hphp(ch)%cval,size(v2b_hphp(ch)%cval),&
          mpi_complex16,mpi_sum,mpi_comm_world,ierror)
     CALL check_mpi(ierror, 'setup_2b_interaction', 'allreduce')
  end DO
  ...
  fock_built = .TRUE.
  interactions_built = .TRUE.
  
end SUBROUTINE setup_2b_interaction


! SUBROUTINE v2b_hphp_cross_2b(fock_mtx0)
!   USE parallel
!   USE single_particle_orbits
!   USE constants  
!   USE operator_storage
...
!   COMPLEX(dpc) :: v2b
!   COMPLEX(dpc), ALLOCATABLE :: fock_tmp(:,:)
...
!      bra_confs = number_2bcross(2,ch)
!      fock_tmp = 0.d0
!      !$omp parallel default(shared) private(bra,ket,ket0, size1, i,b,j,a, v2b) reduction(+:fock_tmp)
...
!            a   = lookup_2bcross_configs(2,ch)%ival2(2,ket)
!            v2b = v2int(i,a,j,b)
!            v2b_hphp_cross(ch)%cval(bra,ket) = v2b_hphp_cross(ch)%cval(bra,ket) + v2b
!            ! Compute fock_mtx
!            IF ( i == j ) then
!               fock_tmp(a,b) = fock_tmp(a,b) + v2b
!               IF ( bra < ket .and. ket <= bra_max ) then
!                  fock_tmp(b,a) = fock_tmp(b,a) + conjg(v2b)
!               end IF
!            end IF
!         end DO
!      end DO
!      !$omp end do
!      !$omp end parallel
!      fock_mtx0 = fock_mtx0 + fock_tmp
!      DO bra = bra_min, bra_max
!         DO ket = bra_min, bra-1
!            v2b_hphp_cross(ch)%cval(bra,ket) = conjg(v2b_hphp_cross(ch)%cval(ket,bra))
!         end DO
!      end DO
!   end DO
!   DEALLOCATE( fock_tmp )
! end SUBROUTINE v2b_hphp_cross_2b
...
SUBROUTINE v2b_hphp_2b(fock_mtx0)
  ...
  COMPLEX(dpc) :: v2b
  COMPLEX(dpc), ALLOCATABLE :: fock_local(:,:,:)

  nthreads = omp_get_max_threads()
  ALLOCATE( fock_local(all_orbit%total_orbits, all_orbit%total_orbits, 0:nthreads-1) )
  ...
  DO ch = 1, channels_2b%number_confs
     ...
     bra_confs = number_2bcross(2,ch)
     !$omp parallel default(shared) private(tid, bra,ket,ket0, size1, i,a,j,b, v2b)
     ...
     DO bra = bra_min, bra_max
        ...
        DO ket0 = bra, size1
           ...
           IF ( i == j ) then
              fock_local(a,b,tid) = fock_local(a,b,tid) + v2b
              IF ( bra < ket .and. ket <= bra_max ) then
                 fock_local(b,a,tid) = fock_local(b,a,tid) + conjg(v2b)
              end IF
           end IF
        end DO
     end DO
     ...
     !$omp end parallel
     DO tid = 0, nthreads-1
        fock_mtx0 = fock_mtx0 + fock_local(:,:,tid)
     END DO
     DO bra = bra_min, bra_max
        DO ket = bra_min, bra-1
...
SUBROUTINE v2b_hhhh_2b(fock_mtx0, e0_2)
  ...
  DO ch = 1, channels_2b%number_confs
     ...
     bra_confs = number_2b(1,ch)
     e_tmp = 0.d0
     !$omp parallel default(shared) private(tid, bra,ket,ket0, size1, i,j,k,l, v2b) reduction(+:e_tmp)
     tid = omp_get_thread_num()
     fock_local(:,:,tid) = 0.d0
     !$omp do schedule(static)
     DO bra = bra_min, bra_max
        ...
        DO ket0 = bra, size1
           ...
           IF ( i == k ) fock_local(j,l,tid) = fock_local(j,l,tid) + v2b
           IF ( i == l ) fock_local(j,k,tid) = fock_local(j,k,tid) - v2b
           IF ( j == l ) fock_local(i,k,tid) = fock_local(i,k,tid) + v2b
           IF ( j == k ) fock_local(i,l,tid) = fock_local(i,l,tid) - v2b
           IF ( bra < ket .and. ket <= bra_max ) then
              IF ( i == k ) fock_local(l,j,tid) = fock_local(l,j,tid) + conjg(v2b)
              IF ( i == l ) fock_local(k,j,tid) = fock_local(k,j,tid) - conjg(v2b)
              IF ( j == l ) fock_local(k,i,tid) = fock_local(k,i,tid) + conjg(v2b)
              IF ( j == k ) fock_local(l,i,tid) = fock_local(l,i,tid) - conjg(v2b)
           end IF
           IF ( bra == ket ) then
              e_tmp = e_tmp + v2b
           end IF
...
SUBROUTINE v2b_hphh_2b(fock_mtx0)
  ...
  DO ch = 1, channels_2b%number_confs
     ...
     bra_confs = number_2b(2,ch)
     ket_confs = number_2b(1,ch)
     !$omp parallel default(shared) private(tid, bra,ket,ket0, i,a,j,k, v2b)
     tid = omp_get_thread_num()
     fock_local(:,:,tid) = 0.d0
     !$omp do schedule(static)
     DO bra = bra_min, bra_max
        ...
        DO ket = 1, ket_confs
           ...
           IF ( i == j ) then
              fock_local(k,a,tid) = fock_local(k,a,tid) + v2b
              fock_local(a,k,tid) = fock_local(a,k,tid) + conjg(v2b)
           else IF ( i == k ) then
              fock_local(j,a,tid) = fock_local(j,a,tid) - v2b
              fock_local(a,j,tid) = fock_local(a,j,tid) - conjg(v2b)
           end IF
        end DO
     end DO
     !$omp end do
...
! SUBROUTINE v2b_hphp_cross_3b(fock_mtx0)
!   USE parallel
!   USE single_particle_orbits
!   USE constants  
!   USE operator_storage
...
!      bra_confs = number_2bcross(2,ch)
!      fock_tmp = 0.d0
!      !$omp parallel default(shared) private(bra,ket,ket0, size1, i,b,j,a,h, v3b,vtmp) reduction(+:fock_tmp)
!      !$omp do schedule(static)
!      DO bra = bra_min, bra_max
!         i   = lookup_2bcross_configs(2,ch)%ival2(1,bra)
!         b   = lookup_2bcross_configs(2,ch)%ival2(2,bra)
...
!               vtmp = vtmp + v3b
!            end DO
!            v2b_hphp_cross(ch)%cval(bra,ket) = v2b_hphp_cross(ch)%cval(bra,ket) + vtmp
!            ! Compute fock_mtx
!            IF ( i == j ) then
!               fock_tmp(a,b) = fock_tmp(a,b) + vtmp/2.d0
!               IF ( bra < ket .and. ket <= bra_max ) then
!                  fock_tmp(b,a) = fock_tmp(b,a) + conjg(vtmp)/2.d0
!               end IF
!            end IF
!         end DO
!      end DO
...
!   end DO
!   DEALLOCATE( fock_tmp )
! end SUBROUTINE v2b_hphp_cross_3b
...
SUBROUTINE v2b_hphp_3b(fock_mtx0)
  ...
  DO ch = 1, channels_2b%number_confs
     ...
     bra_max = mapping_v2b_hphp(iam+1,ch,3)
     bra_confs = number_2b(2,ch)
     !$omp parallel default(shared) private(tid, bra,ket,ket0, size1, i,a,j,b,h, v3b,vtmp)
     tid = omp_get_thread_num()
     fock_local(:,:,tid) = 0.d0
     !$omp do schedule(static)
     DO bra = bra_min, bra_max
        ...
        DO ket0 = bra, size1
           ...
           v2b_hphp(ch)%cval(bra,ket) = v2b_hphp(ch)%cval(bra,ket) + vtmp
           IF ( i == j ) then
              fock_local(a,b,tid) = fock_local(a,b,tid) + vtmp/2.d0
              IF ( bra < ket .and. ket <= bra_max ) then
                 fock_local(b,a,tid) = fock_local(b,a,tid) + conjg(vtmp)/2.d0
              end IF
           end IF
        end DO
     end DO
...
SUBROUTINE v2b_hhhh_3b(fock_mtx0, e0_3)
  ...
  DO ch = 1, channels_2b%number_confs
     ...
     bra_confs = number_2b(1,ch)
     e_tmp = 0.d0
     !$omp parallel default(shared) private(tid, bra,ket,ket0, size1, i,j,k,l,h, v3b,vtmp) reduction(+:e_tmp)
     tid = omp_get_thread_num()
     fock_local(:,:,tid) = 0.d0
     !$omp do schedule(static)
     DO bra = bra_min, bra_max
        ...
        DO ket0 = bra, size1
           ...
           v2b_hhhh(ch)%cval(bra,ket) = v2b_hhhh(ch)%cval(bra,ket) + vtmp
           IF ( i == k ) fock_local(j,l,tid) = fock_local(j,l,tid) + vtmp/2.d0
           IF ( i == l ) fock_local(j,k,tid) = fock_local(j,k,tid) - vtmp/2.d0
           IF ( j == l ) fock_local(i,k,tid) = fock_local(i,k,tid) + vtmp/2.d0
           IF ( j == k ) fock_local(i,l,tid) = fock_local(i,l,tid) - vtmp/2.d0
           IF ( bra < ket .and. ket <= bra_max ) then
              IF ( i == k ) fock_local(l,j,tid) = fock_local(l,j,tid) + conjg(vtmp)/2.d0
              IF ( i == l ) fock_local(k,j,tid) = fock_local(k,j,tid) - conjg(vtmp)/2.d0
              IF ( j == l ) fock_local(k,i,tid) = fock_local(k,i,tid) + conjg(vtmp)/2.d0
              IF ( j == k ) fock_local(l,i,tid) = fock_local(l,i,tid) - conjg(vtmp)/2.d0
           end IF
           IF ( bra == ket ) then
              e_tmp = e_tmp + vtmp/3.d0
           end IF
        end DO
     end DO
     !$omp end do
...
SUBROUTINE v2b_hphh_3b(fock_mtx0)
  ...
  DO ch = 1, channels_2b%number_confs
     ...
     bra_confs = number_2b(2,ch)
     ket_confs = number_2b(1,ch)
     !$omp parallel default(shared) private(tid, bra,ket,ket0, i,a,j,k,h, v3b,vtmp)
     tid = omp_get_thread_num()
     fock_local(:,:,tid) = 0.d0
     !$omp do schedule(static)
     DO bra = bra_min, bra_max
        ...
        DO ket = 1, ket_confs
           ...
           v2b_hphh(ch)%cval(bra,ket) = v2b_hphh(ch)%cval(bra,ket) + vtmp
           IF ( i == j ) then
              fock_local(k,a,tid) = fock_local(k,a,tid) + vtmp/2.d0
              fock_local(a,k,tid) = fock_local(a,k,tid) + conjg(vtmp)/2.d0
           ELSE IF ( i == k ) then
              fock_local(j,a,tid) = fock_local(j,a,tid) - vtmp/2.d0
              fock_local(a,j,tid) = fock_local(a,j,tid) - conjg(vtmp)/2.d0
           end IF
        end DO
     end DO
     !$omp end do
